...
def generate_comparison_report(financials_list):
    ...
    # Формирование таблицы сравнения показателей (выравнивание через форматирование)
    table = f"""ПОКАЗАТЕЛЬ                | {region1:<12} | {region2:<12}
--------------------------|--------------|--------------
Месячная прибыль          | {format_currency(r1['profit']):>12} ₽ | {format_currency(r2['profit']):>12} ₽
Рентабельность            | {r1['profitability']:>5.1f}% ({profit_labels[r1['profitability_level']]}) | {r2['profitability']:>5.1f}% ({profit_labels[r2['profitability_level']]})
Точка безубыточности      | {r1['break_even_children']:>12} детей | {r2['break_even_children']:>12} детей
Конкуренция               | {r1['competition_density']:>5.1f} ({comp_labels[r1['competition_level']]}) | {r2['competition_density']:>5.1f} ({comp_labels[r2['competition_level']]})
Срок окупаемости          | {str(r1['payback_period_month']):>12} мес. | {str(r2['payback_period_month']):>12} мес."""
    
    # Расчет разницы в прибыли между регионами
    profit_diff = r2['profit'] - r1['profit']
    
    # Определение более выгодного региона и формирование аналитического вывода
    if profit_diff > 0:
        # Второй регион более выгоден
        better_region = region2
        # Расчет процентной разницы в прибыли
        diff_pct = abs(profit_diff) / r1['profit'] * 100 if r1['profit'] != 0 else 0
        advantage = f'{region2} выгоднее по всем ключевым метрикам:\n' \
                    f' — выше прибыль (+{profit_diff:,} ₽, +{diff_pct:.0f}%),\n' \
                    f' — ниже точка безубыточности,\n' \
                    f' — слабее конкуренция.'
    elif profit_diff < 0:
        # Первый регион более выгоден
        better_region = region1
        # Расчет процентной разницы в прибыли
        diff_pct = abs(profit_diff) / r2['profit'] * 100 if r2['profit'] != 0 else 0
        advantage = f'{region1} выгоднее по всем ключевым метрикам:\n' \
                    f' — выше прибыль (+{abs(profit_diff):,} ₽, +{diff_pct:.0f}%),\n' \
                    f' — ниже точка безубыточности,\n' \
                    f' — слабее конкуренция.'
    else:
        # Регионы сопоставимы по прибыли
        better_region = 'Оба региона сопоставимы'
        advantage = 'Регионы имеют сопоставимую прибыль. Выбор зависит от локальных факторов (личные связи, помещение и т.д.).'
    
    # Формирование рекомендации на основе прибыльности регионов
    if r1['profit'] <= 0 and r2['profit'] <= 0:
        ...
        # Оба региона рентабельны, рекомендуем более выгодный
        recommendation = f'При прочих равных условиях предпочтение стоит отдать {better_region}.'
...
def generate_overview_report(financials_list):
    ...
    table_lines = ['РЕГИОН         | ПРИБЫЛЬ    | РЕНТАБ. | КОНКУРЕНЦИЯ | ОКУПАЕМОСТЬ']
    table_lines.append('---------------|------------|---------|-------------|-------------')
    
    for r in sorted_regions:
        profit_str = format_currency(r['profit']) + ' ₽'
        rentab_str = f'{r['profitability']:.1f}%'
        comp_str = f'{comp_labels[r['competition_level']]}'
        payback_str = f'{r['payback_period_month']} мес.'
        table_lines.append(f'{r['region']:<14} | {profit_str:>10} | {rentab_str:>7} | {comp_str:>11} | {payback_str:>10}')
